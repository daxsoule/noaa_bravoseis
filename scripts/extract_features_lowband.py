#!/usr/bin/env python3
"""
extract_features_lowband.py — Re-extract features for low-band events
with a 1–14 Hz bandpass applied BEFORE computing spectrogram features.

This removes fin whale 20 Hz contamination and isolates seismic energy
(T-phases, local earthquakes) in the sub-14 Hz band.

Usage:
    uv run python extract_features_lowband.py
    uv run python extract_features_lowband.py --workers 4
    uv run python extract_features_lowband.py --resume

Output:
    outputs/data/event_features_lowband.parquet
"""
...
CHECKPOINT_DIR = DATA_DIR / "feature_checkpoints_lowband"
...
# Longer window for low frequencies — need good resolution below 14 Hz
# nperseg=2048 gives ~0.49 Hz bins at 1 kHz (vs ~0.98 Hz with 1024)
NPERSEG = 2048
NOVERLAP = 1024
...
PRE_PICK_SEC = 5.0
POST_PICK_SEC = 10.0
WINDOW_SEC = PRE_PICK_SEC + POST_PICK_SEC  # 15s

# Frequency bands for feature extraction (1–14 Hz, ~2 Hz each)
FREQ_BANDS = [
    (1, 3),    # ultra-low
    (3, 5),    # T-phase low
    (5, 7),    # T-phase mid
...
def extract_features_from_patch(freqs, times, Sxx_dB, Sxx_linear,
    ...
    # --- Band powers (6 bands, ~2 Hz each within 1–14 Hz) ---
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-extract features for low-band events with 1-14 Hz bandpass")
    ...
    print("=" * 60)
    print("BRAVOSEIS Low-Band Feature Extraction (1–14 Hz)")
    ...
    # Load catalogue — only low-band events
    ...
    cat = cat[cat["detection_band"] == "low"].copy()
    print(f"Low-band events: {len(cat):,}")
    ...
    print(f"\n{'=' * 60}")
    print(f"Low-band feature extraction complete")
    ...
    outpath = DATA_DIR / "event_features_lowband.parquet"
