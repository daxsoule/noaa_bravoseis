#!/usr/bin/env python3
"""
make_lowband_panels.py — Generate review panels for lowband (1–14 Hz) clusters.

Uses the new umap_coordinates_lowband.parquet clusters and shows spectrograms
focused on the 0–20 Hz range for better visibility of sub-14 Hz signals.

Usage:
    uv run python make_lowband_panels.py                    # all clusters, stratified
    uv run python make_lowband_panels.py --cluster lowband_2
    uv run python make_lowband_panels.py --strategy random
    uv run python make_lowband_panels.py --n 15
"""
...
# === Display parameters ===
NPERSEG_DISPLAY = 2048   # Good low-freq resolution (~0.49 Hz)
NOVERLAP_DISPLAY = 1536  # 75% overlap for smooth display
SPEC_FMAX = 20           # Show 0–20 Hz for context above 14 Hz cutoff
WINDOW_SEC = 15
PAD_SEC = 5.0
...
def load_data():
    """Load lowband UMAP clusters + catalogue + features."""
...
def plot_single_event(cluster_id, panel_num, ev, snip, cluster_size,
    ...
    # Show 14 Hz cutoff line
    ax_spec.axhline(14.0, color="cyan", linewidth=0.7,
                    linestyle=":", alpha=0.6, label="14 Hz cutoff")
    ax_spec.set_ylabel("Frequency (Hz)", fontsize=10)
    ax_spec.set_xlabel("Time (s)", fontsize=10)
    ax_spec.set_title(f"Spectrogram — 0–{SPEC_FMAX} Hz (nperseg={NPERSEG_DISPLAY})",
                      fontsize=10, loc="left")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate review panels for lowband (1-14 Hz) clusters")
    parser.add_argument("--cluster", type=str, default=None,
                        help="Single cluster ID (e.g., lowband_2). "
                             "Default: all non-mega clusters")
    parser.add_argument("--strategy", type=str, default="stratified",
                        choices=["stratified", "random"])
    parser.add_argument("--n", type=int, default=15,
                        help="Panels per cluster (default: 15)")
    parser.add_argument("--all", action="store_true",
                        help="Include mega-cluster (lowband_5)")
    args = parser.parse_args()

    FIG_DIR.mkdir(parents=True, exist_ok=True)

    print("Loading lowband data...")
    df = load_data()

    if args.cluster:
        clusters = [args.cluster]
    else:
        # All clusters sorted by size (smallest first), skip mega-cluster
        # unless --all
    ...
    print(f"\nDone. {len(clusters)} clusters × {args.n} panels = "
