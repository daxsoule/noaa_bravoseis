#!/usr/bin/env python3
"""
cluster_highband.py — UMAP + HDBSCAN clustering of highband (30–450 Hz) events.

Targets icequake and vessel noise separation. No whale filter needed since
whale energy is below 30 Hz.

Usage:
    uv run python scripts/cluster_highband.py
    uv run python scripts/cluster_highband.py --min-cluster-size 300
    uv run python scripts/cluster_highband.py --skip-stability
"""
...
FIG_DIR = OUTPUT_DIR / "figures" / "exploratory" / "clustering" / "phase3_highband"

# === Feature columns (from extract_features_highband.py — 6 bands) ===
FEATURE_COLS = (
    [f"band_power_{i}" for i in range(6)]
    + ["peak_freq_hz", "peak_power_db", "bandwidth_hz", "duration_s",
...
def plot_umap_clusters(embedding, labels, outpath):
    ...
    for i, cid in enumerate(cluster_ids):
        ...
        ax.scatter(embedding[mask, 0], embedding[mask, 1],
                   ...
                   label=f"highband_{cid} (n={n_in:,})", rasterized=True)

    ax.set_xlabel("UMAP 1", fontsize=12)
    ax.set_ylabel("UMAP 2", fontsize=12)
    ax.set_title(
        f"Highband (30–450 Hz) — {n_clusters} clusters, "
...
def plot_umap_features(embedding, df_valid, outpath):
    ...
    fig.suptitle("Highband (30–450 Hz) — UMAP by Feature",
                 fontsize=14, fontweight="bold")
    fig.tight_layout()
    fig.savefig(outpath, dpi=300, facecolor="white")
    plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description="UMAP+HDBSCAN clustering of highband events")
    ...
    if not args.skip_stability:
        ...
        stab_path = DATA_DIR / "cluster_stability_highband.csv"
    ...
    for cid in sorted(set(labels) - {-1}):
        n = (labels == cid).sum()
        print(f"    highband_{cid}: {n:,}")

    # Build cluster IDs
    cluster_ids_str = [
        f"highband_{cid}" if cid >= 0 else "highband_noise"
        for cid in labels
    ]
    ...
    # Save
    outpath = DATA_DIR / "umap_coordinates_highband.parquet"
    result_df.to_parquet(outpath, index=False)
    print(f"\nSaved: {outpath} ({len(result_df):,} events)")

    # Figures
    plot_umap_clusters(embedding, labels,
                       FIG_DIR / "umap_highband_cluster_map.png")
    plot_umap_features(embedding, df_valid,
                       FIG_DIR / "umap_highband_feature_maps.png")

    # Cluster characterization
    print("\n=== Cluster characterization ===")
    for cid in sorted(set(cluster_ids_str)):
        sub_mask = result_df["cluster_id"] == cid
        sub = df_valid[sub_mask]
        n = len(sub)
        print(f"\n{cid} (n={n:,}):")
        for col in ["peak_freq_hz", "snr", "spectral_slope", "duration_s",
                     "bandwidth_hz", "spectral_centroid_hz"]:
            print(f"  {col}: median={sub[col].median():.1f}, "
                  f"mean={sub[col].mean():.1f}")
        # Band power profile
        bands = [f"band_power_{i}" for i in range(6)]
        band_labels = ["30-60", "60-100", "100-160", "160-250", "250-350", "350-450"]
        bp = [sub[b].median() for b in bands]
        peak_band_idx = np.argmax(bp)
        print(f"  Peak band: {band_labels[peak_band_idx]} Hz")
        print(f"  Band powers: {[f'{v:.1f}' for v in bp]}")
